...
final class MultitouchDialSource {
    ...
    // The surface is far wider than it is deep, so angles computed straight from
    // normalized coordinates would change faster through the horizontal than the
    // vertical and the dial's gain would depend on hand position.
    ...
    func start(handler: @escaping Handler) -> Bool {
        ...
        if let first = devices.first {
            let size = MultitouchSupport.surfaceSize(of: first)
    ...
    fileprivate func process(touches: [MTTouch]) {
...
private let multitouchContactCallback: MTContactCallback = { _, touchesPtr, count, _, _ in
    ...
    DispatchQueue.main.async {
        activeDialSource?.process(touches: touches)
    }
    return 0
}
